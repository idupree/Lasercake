...
typedef time_steward_system::time_steward<> time_steward;
...
class circle : public spatial_entity {
public:
  circle(poly_fd_vector center, poly radius):center(center),radius(radius){}
  ...
  public:
    ...
    void operator()(time_steward::accessor* accessor)const override {
      ...
      auto c1 = accessor->get_mut(id1);
      const space_coordinate d = circle::distish(*c0,*c1)(accessor->now());
      ...
      // simple hack!
      if (d < 0) {
        c0->overlaps.insert(id1);
        c1->overlaps.insert(id0);
      }
      else {
        c0->overlaps.erase(id1);
        c1->overlaps.erase(id0);
      }
  ...
  void update_acceleration(time_steward::accessor const* accessor) {
