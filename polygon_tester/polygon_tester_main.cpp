...
for (int i = 0; i < 50; ++i) {
  ...
  while ( !done ) {
    ...
    foreach (floating_line& foo , lines) {
    ...
    foreach (floating_poly& foo , polys) {
      if (foo.collided) glColor3f(1.0,1.0,0.0);
      else              glColor3f(0.0,1.0,0.0);
      glBegin(GL_POLYGON);
        foreach (vector3<int64_t>& v , foo.vertices)
	...
	++frame;
    foreach (floating_line& foo , lines) {
      foreach (vector3<int64_t>& v , foo.ends) --v.z;
      foo.collided = false;
    }
    foreach (floating_poly& foo , polys) {
      foreach (vector3<int64_t>& v , foo.vertices) ++v.z;
      foo.collided = false;
    }
    foreach (floating_line& foo , lines) {
      foreach (floating_poly& bar , polys) {
